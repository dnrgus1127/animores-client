...
export const CalendarDropDownHeader: React.FC<DropDownHeaderProps> = ({ styles }) => {
  ...
  const dateString = useMemo(
    () => getDateString(pickerType, year, month),
  ...
  const onPressItem = (data: number) => {
    if (pickerType === PickerType.Year) {
      ...
      setMonth(data);
      const mm = String(data).padStart(2, "0");
      setSelectedDate(`${year}-${mm}-01`);
    }
  ...
  return (
    <View style={[dropDownHeaderStyles.container]}>
      ...
      {pickerType !== PickerType.Day && (
        <Pressable
          onPress={() => setPickerType(PickerType.Day)}
...
const DatePickerBody: React.FC<MonthPicker> = ({ year, month, type, onPress }) => {
  ...
  return (
    <View style={dropDownHeaderStyles.monthPicker}>
      {itemList.map(item => {
        ...
        return (
          <TouchableOpacity
            key={item}
            style={dropDownHeaderStyles.box}
            onPress={() => {
              onPress(item);
            }}
          >
